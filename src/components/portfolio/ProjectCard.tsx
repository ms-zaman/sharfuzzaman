...
import { formatProjectDateRange, getProjectStatusColor, getCategoryDisplayName } from '~/utils/portfolio'
    ...
    return (
      ...
      >
        ...
        <div className={cn(
          ...
          layout === 'grid' ? 'aspect-video' : 'md:w-80 md:h-48 md:flex-shrink-0'
        )}>
          {project.heroImage ? (
            <img
        ...
        <div className={cn(projectCardVariants.size[size], 'flex-1')}>
          <CardHeader className="p-0">
            ...
            <div className="flex items-center justify-between text-sm text-slate-500 mb-2">
              <span className="font-medium">{getCategoryDisplayName(project.category)}</span>
          ...
          {/* Metrics */}
          {showMetrics && project.results && project.results.length > 0 && (
            <div className="mt-4 pt-4 border-t border-slate-200">
              <div className="grid grid-cols-2 gap-4">
                {project.results.slice(0, 2).map((metric, index) => (
